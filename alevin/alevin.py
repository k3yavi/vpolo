...
def read_quants_bin(base_location):
    ...
    if not base_location.exists():
        print("{} directory doesn't exist", base_location)
        exit(1)
    if not base_location.is_dir():
        print("{} is not a directory", base_location)
        exit(1)
    
    quant_file = base_location + "/quants_mat.gz"
    if not quant_file.exists():
        print("quant file {} doesn't exist", quant_file)
        exit(1)
        
    cb_file = base_location + "/quants_mat_rows.txt"
    if not quant_file.exists():
        print("quant file's index: {} doesn't exist", cb_file)
        exit(1)

    gene_file = base_location + "/quants_mat_cols.txt"
    if not quant_file.exists():
        print("quant file's header: {} doesn't exist", gene_file)
        exit(1)
...
def read_eq_bin( base_location ):
    ...
    if not eq_file.exists():
        print("eqclass file {} doesn't exist", eq_file)
        exit(1)
        
    order_file = base_location + "/cell_eq_order.txt"
    if not order_file.exists():
        print("cell order file {} doesn't exist", order_file)
        exit(1)
...
def read_bfh(base_location, t2gFile, retype="counts"):
    ...
    if not bfh_file.exists():
        print("bfh file {} doesn't exist", bfh_file)
        exit(1)
        
    t2g_file = Path(t2gFile)
    if not t2g_file.exists():
        print("t2g file {} doesn't exist", t2g_file)
        exit(1)
